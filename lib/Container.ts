...
export class Container {
	 ...
	 * @private
	 */
	private contextual: Map<string, Map<string, string>> = new Map();
	 ...
	 * ```
	 */
	bind<T>(abstract: string, concrete: Constructor<T> | ServiceFactory<T>, shared = false): void {
	 ...
	 * ```
	 */
	singleton<T>(abstract: string, concrete: Constructor<T> | ServiceFactory<T>): void {
	 ...
	 * @returns void
	 */
	addContextualBinding(concrete: string, abstract: string, implementation: string): void {
	 ...
	 */
	getContextualConcrete(abstract: string, concrete: string): string | null {
	...
	make<T>(abstract: string, parameters: any[] = []): T {
		...
		const concrete = this.getConcrete<T>(abstract);
		...
		const instance = this.build<T>(concrete as Constructor<T>, parameters);
	 ...
	 */
	private getConcrete<T>(abstract: string): Constructor<T> | ServiceFactory<T> {
	 ...
	 */
	private isBuildable(concrete: any, abstract: string): boolean {
	 ...
	 */
	private build<T>(concrete: Constructor<T>, parameters: any[] = []): T {
	 ...
	 */
	private resolveDependencies<T>(concrete: Constructor<T>): any[] {
		...
		// Đây là một mock implementation đơn giản
		return [];
	}
...
export class ContextualBindingBuilder {
	 ...
	 */
	needs(abstract: string): ContextualBindingNeedsBuilder {
...
export class ContextualBindingNeedsBuilder {
	 ...
	 * @returns void
	 */
	give(implementation: string): void {
